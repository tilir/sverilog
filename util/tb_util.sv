// Small shared scoreboard for the self-checking example testbenches.
package tb_util_pkg;
  class scoreboard;
    int unsigned checks;
    ...
    function void end_test(input string name);
      ...
      if (section_failures == 0)
        $display("PASS %-18s %0d checks", name, section_checks);
      else
    ...
    function void finish_tests(input string suite);
      ...
      if (failures == 0) begin
        ...
        $display("FAIL %s: %0d checks, %0d failures", suite, checks, failures);
